...
describe('validateTextarea', () => {
  ...
  it('returns custom error for textarea exceeding maxLength', () => {
    const result = validateTextarea('This is a very long textarea that exceeds the maximum length.', true, 20, ['Custom error 1', 'Custom error 2', 'Custom error 3']);
    expect(result).toEqual({ isValid: false, errorMsg: 'Custom error 1' });
  });

  it('throws error for invalid errorMsg parameter', () => {
    expect(() => validateTextarea('This is a valid textarea.', true, 50, [123 as any])).toThrow('All values within the array must be strings or null/undefined.');
  });
});
